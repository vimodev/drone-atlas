...
function filterChanged() {
  ...
  loop: for (const marker of imageMarkers.markers) {
    ...
    // Check height
    if (heightOperator == 'ge' && marker.image.altitude < height) {
      marker.display(false)
      continue loop
    }
    if (heightOperator == 'le' && marker.image.altitude > height) {
      marker.display(false)
      continue loop
    }
  }
...
function calculateMarkerPosition(p) {
  ...
  if (positions[p.longitude] != undefined && positions[p.longitude][p.latitude] != undefined) {
    ...
    for (let x_offset = -1; x_offset <= 1; x_offset++) {
      for (let y_offset = -1; y_offset <= 1; y_offset++) {
        let new_x = p.longitude + x_offset * position_step
        let new_y = p.latitude + y_offset * position_step
    ...
    if (!fits) {
      x = p.longitude - position_step + (2 * Math.random() * position_step)
      y = p.latitude - position_step + (2 * Math.random() * position_step)
    }
  } else {
    // If position was free, just place it
    if (positions[p.longitude] == undefined) positions[p.longitude] = {}
    positions[p.longitude][p.latitude] = true
    x = p.longitude
    y = p.latitude
  }
  return {x, y}
...
function handleDatapoints(video, points) {
  ...
  // Go over all points
    ...
    if (p_prev != null && p_prev.longitude == p.longitude && p_prev.latitude == p.latitude) continue
...
function handleImages(images) {
  ...
  for (const image of images) {
    ...
    if (prev_image != null
      && Math.abs(prev_image.longitude - image.longitude) < 0.00001
      && Math.abs(prev_image.latitude - image.latitude) < 0.00001) {
      continue
    }
    // Set color
    let filter = `brightness(${Math.random() * 0.75 + 1})hue-rotate(${Math.random()}turn)saturate(4)`
    ...
    marker = new OpenLayers.Marker(
      new OpenLayers.LonLat(image.longitude, image.latitude).transform(fromProjection, toProjection),
      icon.clone()
    )
    image.marker = marker
    marker.image = image
...
async function fetchVideoDatapoints(video) {
  return await new Promise((resolve) => {
    fetch("/api/point/from?videoId=" + video.id)
